    ...
    (function (cast) {
        ...
        var Session = /** @class */ (function () {
            ...
            Session.prototype.setReceiverVolumeLevel = function (newLevel, successCallback, errorCallback) {
                //this is used in: ChromecastPlayer.ts: this._session.setReceiverVolumeLevel(level, () => {
                //this.nativeSession.setReceiverVolumeLevel(newLevel, chromecastCallbackHandler.register(successCallback, errorCallback))
                try {
                    var param = {};
                    param["newLevel"] = newLevel;
                    var callbackId = chromecastCallbackHandler.register(successCallback, errorCallback);
                    param["callbackId"] = callbackId;
                    webkit.messageHandlers.sessionBridgeSetReceiverVolumeLevel.postMessage(JSON.stringify(param));
                }
                catch (err) {
                    console.log('Error while trying sessionBridgeSetReceiverVolumeLevel. The native context does not exist.', err);
                }
            };
            ;
